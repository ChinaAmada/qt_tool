...
void myTcpServer::incomingConnection(int handle)
{
    ...
    connect(client,SIGNAL(ClientDisConnect(int,QString,int)),this,SIGNAL(ClientDisConnect(int,QString,int)));

    emit ClientConnect(handle, client->peerAddress().toString().mid(client->peerAddress().toString().indexOf("192")),client->peerPort());
...
void myTcpServer::CloseAllClient()
{
    ...
    foreach(myTcpClient* p,ClientList)
        p->close();
    clientCount=0;
    ClientID.clear();
